...
**Key Characteristics:**
- Single accent color: `{colors.primary}` (#10b981 — "Vert") carries every primary CTA, the search orb, the heart save state, and the brand wordmark. Used scarcely — most pages are 90% white + ink with one or two Vert moments.
...
- Pill-shaped global search bar: white surface, fully rounded (`{rounded.full}`), divided by 1px hairlines into Where / When / Who segments, terminated by a circular Vert search orb (`{component.search-orb}`).
...
### Brand & Accent
- **Vert** (`{colors.primary}` — #10b981): The single brand color. Used for primary CTA backgrounds (Reserve, Continue), the search orb, the heart save state on property cards, and inline brand links. The most recognizable color in consumer travel.
- **Vert Active** (`{colors.primary-active}` — #059669): The press / pointer-down variant — slightly more saturated. Used on `{component.button-primary-active}`.
- **Vert Disabled** (`{colors.primary-disabled}` — #d1fae5): A pale tint used on disabled CTAs.
...
- **On Primary** (`{colors.on-primary}` — #ffffff): White text on Vert CTAs.
